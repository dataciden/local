...
# Import OS modules for environment variables
import os
​
...
app.config['MYSQL_DATABASE_HOST'] = os.getenv('DB_URI','phonebook-app-db-server.cbanmzptkrzf.us-east-1.rds.amazonaws.com')
...
def insert_person(name, number):
    query = f"""
    SELECT * FROM phonebook WHERE number like '{number.strip().lower()}';
    """
    cursor.execute(query)
    row = cursor.fetchone()
    if row is not None:
        return f'Person with number {row[1].title()} already exists.'
​
    insert = f"""
